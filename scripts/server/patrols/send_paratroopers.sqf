...
if ({(Backpack _x) == ""} foreach (units _para_group) || {(Backpack _x) != "B_Parachute"} foreach (units _para_group)) then {
	{removeBackpack _x} foreach (units _para_group);
	{_x addBackpack "B_Parachute"} foreach (units _para_group);
};
